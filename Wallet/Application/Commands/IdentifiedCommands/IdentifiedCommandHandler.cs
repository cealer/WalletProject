        ...
        public async Task<R> Handle(IdentifiedCommand<T, R> message, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    switch (command)
                    {
                        case CreateWalletCommand createOrderCommand:
                            idProperty = nameof(createOrderCommand.UserId);
                            commandId = createOrderCommand.UserId.ToString();
                            break;

                        case CreateDepositCommand createDepositCommand:
                            idProperty = nameof(createDepositCommand.UserId);
                            commandId = createDepositCommand.UserId.ToString();
                            break;

                        case CreatePaymentCommand paymentCommand:
                            idProperty = nameof(paymentCommand.UserId);
                            commandId = paymentCommand.UserId.ToString();
                            break;
